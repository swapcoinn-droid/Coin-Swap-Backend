...
walletRouter.post("/deposit", requireAuth, asyncHandler(async (req, res, next) => {
...
walletRouter.post("/withdraw", requireAuth, asyncHandler(async (req, res, next) => {
    const { amount, currency = "COP" } = req.body;
    if (!amount || amount <= 0) return next(badRequest("Monto inválido"));
...
walletRouter.post("/exchange", requireAuth, asyncHandler(async (req, res, next) => {
